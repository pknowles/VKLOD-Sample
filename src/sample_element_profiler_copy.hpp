...
// Forked from nvpro_core to make m_showWindow accessible
...
public:
  ...
  bool isWindowShown() const { return m_showWindow; }

  void toggleWindowShown() { m_showWindow = !m_showWindow; }
...
private:
  ...
  static ImColor uintToColor(uint32_t v)
  {
    ...
    float r = float(hashed & 0xFF) / 255.f;
    hashed  = hashed >> 8;
    float g = float(hashed & 0xFF) / 255.f;
    hashed  = hashed >> 8;
    float b = float(hashed & 0xFF) / 255.f;
  ...
  void renderLineChart()
  {
    ...
    for(size_t i = 0; i < m_node.child.size(); i++)
    {
      ...
      if(m_node.child[i].entry)
      {
        ...
        {
          uint32_t index = uint32_t(m_node.child[i].entry->gpuTime.index - m_node.child[i].entry->gpuTime.numValid + j)
                           % m_data->numAveraging;
    ...
    {
      uint32_t index = uint32_t(m_data->cpuTime.index - m_data->cpuTime.numValid + j) % m_data->numAveraging;
      cpuTimes[j]    = float(m_data->cpuTime.times[index] / 1000.0);
      avgCpuTime += cpuTimes[j];
    }
    if(m_data->cpuTime.numValid > 0)
    {
      avgCpuTime /= float(m_data->cpuTime.numValid);
    }
    if(maxY == 0.f)
    {
      maxY = avgCpuTime;
  ...
  void addSettingsHandler()
  {
    ...
    iniHandler.ClearAllFn = [](ImGuiContext*, ImGuiSettingsHandler*) {};
    iniHandler.ApplyAllFn = [](ImGuiContext*, ImGuiSettingsHandler*) {};
